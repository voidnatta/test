...
function CookingCounter:on_interact(player)
    ...
    if player:has_item_object() and not self:has_item_object() then
        ...
        if item_obj:can_be_contained() then
            ...
            local cooking_sound = love.audio.newSource("assets/sfx/cooking_without_cover_01.ogg", "static")
            cooking_sound:setVolume(0.5)
            cooking_sound:play()
    ...
    elseif self:has_item_object() then
        if player:has_item_object() then
            print("Player is already holding something. Cannot take cooked item.")
            return
        end
